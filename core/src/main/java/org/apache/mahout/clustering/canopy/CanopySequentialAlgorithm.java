...
public class CanopySequentialAlgorithm {
  ...
  public CanopyConfiguration run(CanopyConfiguration canopyConfiguration) throws ClassNotFoundException, IllegalAccessException, InstantiationException, IOException {
    ...
    DistanceMeasure measure = ccl.loadClass(canopyConfiguration.getDistanceMeasureClassName()).asSubclass(DistanceMeasure.class).newInstance();

    CanopyClusterer clusterer = new CanopyClusterer(measure, canopyConfiguration.getT1(), canopyConfiguration.getT2());
    ...
    Path path = new Path(canopyConfiguration.getCanopyPath(), "part-r-00000");
    ...
    fs = FileSystem.get(canopyConfiguration.getCanopyPath().toUri(), canopyConfiguration.getConfiguration());
    status = fs.listStatus(canopyConfiguration.getCanopyPath(), new OutputLogFilter());
    ...
    if (canopyConfiguration.runsClustering()) {
      ...
      for (FileStatus s : status) {
        SequenceFile.Reader reader = new SequenceFile.Reader(fs, s.getPath(), canopyConfiguration.getConfiguration());
        writer = new SequenceFile.Writer(fs, canopyConfiguration.getConfiguration(), new Path(canopyConfiguration.getPointsPath(), "part-m-" + part++), IntWritable.class, WeightedVectorWritable.class);
        try {
          Writable key = reader.getKeyClass().asSubclass(Writable.class).newInstance();
          VectorWritable vw = reader.getValueClass().asSubclass(VectorWritable.class).newInstance();
          while (reader.next(key, vw)) {
